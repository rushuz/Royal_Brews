...
function triggerSiteAnimations() {
    ...
    .add({ targets: '.navbar-brand', translateX: [-50, 0], opacity: [0, 1], duration: 1000 })
    .add({ targets: '.navbar-nav .nav-item', opacity: [0, 1], translateY: [-20, 0], delay: anime.stagger(100) }, '-=600');
}
...
function handleLoginSubmit(event) {
    ...
    if (email && pass) {
        ...
        // 3. Reveal the Main Page Content
        revealMainPage();
    }
}
...
// Update your existing handleAuthSubmit (Signup) to use the same reveal function
function handleAuthSubmit(event) {
    event.preventDefault();
    
    // ... (Your existing validation logic for matching passwords/email) ...
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handlePieceClick(index) {
        if (selectedIndex === null) {
            ...
            // Swap pieces
            [currentOrder[selectedIndex], currentOrder[index]] = [currentOrder[index], currentOrder[selectedIndex]];
